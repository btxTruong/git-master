...
## Acceptance Criteria
- [x] RestoreArchiveToWorkingTree performs preflight check with `git apply --check`
- [x] Optional backup creates stash of affected paths before applying
- [x] Three-way merge attempted when base blobs available
- [x] Fallback to --reject mode on conflicts
- [x] Restoration does not modify Git index by default
- [x] Detailed error messages for apply failures
- [x] Reject files (.rej) surfaced to user if created
- [x] Rollback via backup stash if restoration fails
